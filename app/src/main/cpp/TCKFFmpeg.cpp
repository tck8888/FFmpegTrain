...
void *task_prepare(void *args) {
    TCKFFmpeg *ffmpeg = static_cast<TCKFFmpeg *>(args);
    ffmpeg->_prepare();
    return 0;
}

TCKFFmpeg::TCKFFmpeg(JavaCallHelper *callHelper, const char *dataSource) {
...
void TCKFFmpeg::prepare() {
    //创建一个线程
    pthread_create(&pid, 0, task_prepare, this);
}

void TCKFFmpeg::_prepare() {
    // 初始化网络 让ffmpeg能够使用网络
...
void TCKFFmpeg::setRenderFrameCallback(RenderFrameCallback callback){
